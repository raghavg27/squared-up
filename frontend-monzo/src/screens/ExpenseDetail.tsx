  ...
  return (
    <div className="min-h-screen bg-paper pb-10">
      <PageBanner
        ...
        sub={gName}
        action={
          <button onClick={() => nav(`/expense/${expId}/edit`)} aria-label="Edit" className="w-10 h-10 flex items-center justify-center active:scale-95 transition-transform">
      ...
      <main className="monzo-sheet mx-3 -mt-9 px-6 pb-8 flex flex-col gap-5">
        ...
        <div className="flex flex-col items-center text-center pt-5">
          <span className="w-16 h-16 rounded-xl bg-surface shadow-soft text-primary flex items-center justify-center">
            <Icon name={cat.icon} style={{ fontSize: 30 }} />
          </span>
          <h2 className="font-heading text-[22px] font-extrabold text-ink mt-3">{exp.description}</h2>
          <p className="font-body text-[13px] font-semibold text-neutral-600 mt-1">Added by {addedBy === me?.id ? 'you' : addedBy ? name(addedBy) : '—'} on {date}</p>
          <p className="font-heading text-[40px] font-extrabold text-ink tracking-[-1.5px] tnum mt-2">{rupees(exp.amount_paise)}</p>
        </div>
        {/* Who Paid */}
        ...
        <Card icon="chat_bubble" title="Comments">
          ...
          <div className="flex items-center gap-2 mt-2">
            ...
            <button
              ...
              className="w-11 h-11 shrink-0 rounded-full bg-primary text-on-primary shadow-soft flex items-center justify-center active:scale-95 transition-transform disabled:opacity-50"
            >
  ...
  return (
    <section>
      <div className="flex items-center gap-2 mb-2">
        <Icon name={icon} className="text-outline" style={{ fontSize: 20 }} />
        <h3 className="font-body text-[16px] font-semibold text-neutral-600">{title}</h3>
      </div>
      {children}
    </section>
  );
